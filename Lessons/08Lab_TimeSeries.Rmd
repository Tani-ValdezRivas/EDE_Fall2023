...
is.na(BR_complete) # can do summary() as it will show na at the end but should not do this in the assignment
# need to create a monthly sequence, which will be a data frame, use the join function 
...
Rainfall_ts <- ts(BR_complete$Rainfall,
                   start=c(1901,1),   # can do start = c multiple ways
                   frequency=12) # B/c monthly, frequency must match
...
Rainfall_decomp <- stl(Rainfall_ts,s.window = "periodic") #defaulting
plot(Rainfall_decomp)
# the bigger the grey bar on the plot thats on right hand side, the smaller the impact it has
...
Rainall_decomp_Components <- as.data.frame(Rainfall_decomp$time.series[,1:3])
