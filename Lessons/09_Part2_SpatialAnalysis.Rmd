...
neuse_sf<- st_read(here('./Data/Spatial/NCHUC8.shp')) %>% 
...
ggplot() +
  ...
  geom_sf(data = neuse_sf, fill = 'black')
...
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) # can add , dist = 100000 
...
intersecting_huc_tri <- huc8_sf_utm[tri_sf,] # can add , dist = 100000 
mapView(intersecting_huc_tri)
